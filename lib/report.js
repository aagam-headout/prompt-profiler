...
function hourlyChart(hours) {
  ...
  const cols = hours
    ...
    .join('');
  const ticks = [0, 6, 12, 18, 23].map((hn) => `<span style="flex:1;text-align:center">${hn}</span>`).join('');
...
<div class="wrap">
  <header>
    ...
    <p class="lede">A candidate's full local Claude Code and Cursor session history, distilled into prompting-style signals for human review. This file is self-contained: no server, no network.</p>
  ...
  const distTechSection = `
    <div class="two" style="margin-top:16px">
      ...
      <div><div class="hdr">Technicality — top terms</div><div class="card">${
        techItems.length ? bars(techItems, { mono: true }) : '<div class="sub">No technical terms detected.</div>'
      }</div></div>
    </div>`;
  ...
  const hourly = result.hourly && result.hourly.some((h) => h.count) ? hourlyChart(result.hourly) : '';
  ...
  const behaviorItems = result.behavior.map((b) => ({ label: b.label, value: b.pct, display: b.pct + '%' }));
  const tagsHtml = result.tags.length
    ? result.tags
        .map((t) => `<span class="chip">${esc(t.name)} <span class="pct">${t.count} · ${t.pct}%</span></span>`)
        .join('')
