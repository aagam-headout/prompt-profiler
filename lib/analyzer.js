...
const QUESTION_WORDS = ['how', 'why', 'what', 'when', 'where', 'which', 'who', 'can', 'could', 'should', 'would', 'is', 'are', 'does', 'do'];
const POLITE = ['please', 'thanks', 'thank you', 'appreciate', 'kindly'];
const CORRECTION = ['no,', 'nope', "that's wrong", 'thats wrong', 'not what', 'actually', 'instead', 'undo', 'revert', 'i meant', 'incorrect', "doesn't work", 'still broken', 'again', 'wrong'];
const REASONING = ['because', 'since', 'therefore', 'so that', 'in order to', 'the reason', 'given that', 'assuming', 'trade-off', 'tradeoff', 'edge case', 'constraint', 'however', 'whereas', 'depends'];
...
const TECH = [
  // core / language
  'function', 'async', 'await', 'closure', 'recursion', 'generic', 'enum', 'interface',
  'immutable', 'polymorphism', 'abstraction', 'higher-order', 'coroutine', 'iterator',
  // web / frontend
  'component', 'hook', 'state', 'props', 'render', 'hydration', 'ssr', 'csr', 'memoization',
  'reconciliation', 'virtual dom', 'debounce', 'throttle', 'lazy load', 'accessibility',
  // backend / api
  'endpoint', 'middleware', 'webhook', 'rest', 'graphql', 'grpc', 'websocket', 'rate limit',
  'idempotent', 'pagination', 'serialization', 'api gateway', 'microservice', 'load balancer',
  // data
  'schema', 'query', 'migration', 'transaction', 'index', 'foreign key', 'orm', 'sharding',
  'replication', 'normalization', 'cache', 'cache invalidation',
  // concurrency / performance
  'concurrency', 'mutex', 'deadlock', 'race condition', 'thread', 'latency', 'throughput',
  'memoize', 'bottleneck', 'event loop', 'backpressure', 'queue',
  // reliability / errors
  'error', 'exception', 'stack trace', 'memory leak', 'timeout', 'retry', 'fallback',
  'circuit breaker', 'idempotency', 'graceful shutdown', 'null pointer',
  // architecture / patterns
  'dependency injection', 'singleton', 'factory', 'observer', 'pub/sub', 'pipeline',
  'state machine', 'monorepo', 'feature flag', 'refactor', 'abstraction layer',
  // infra / devops
  'docker', 'kubernetes', 'container', 'terraform', 'ci/cd', 'deploy', 'autoscale', 'ingress',
  'env var', 'cron',
  // security
  'auth', 'oauth', 'jwt', 'token', 'sanitize', 'injection', 'xss', 'csrf', 'encryption', 'hashing',
  // tooling / vcs
  'commit', 'branch', 'merge', 'rebase', 'cherry-pick', 'squash', 'lint', 'typecheck',
  'bundle', 'tree shaking', 'regex', 'mock', 'fixture', 'assertion', 'coverage', 'snapshot',
  'dependency',
];
...
const IMPERATIVE = ['add', 'fix', 'create', 'make', 'implement', 'update', 'remove', 'delete', 'change', 'build', 'write', 'refactor', 'move', 'rename', 'use', 'run', 'install', 'set', 'generate', 'convert', 'update', 'check', 'show'];
const URGENCY = ['asap', 'urgent', 'quickly', 'right now', 'immediately', 'hurry', 'need this now', 'time-sensitive'];
const FRUSTRATION = ['still not', 'still broken', 'ugh', 'wtf', 'why is this', "doesn't work", 'not working', 'frustrat', 'annoying', 'stuck', 'come on', 'seriously'];
const AUTONOMY = ['you decide', 'your call', 'whatever you think', 'up to you', 'use your judgment', 'best approach', 'you choose', 'what do you think', 'your recommendation', 'whatever is best'];
const PLANNING = ['first', 'then', 'step 1', 'step 2', 'plan', 'approach', 'strategy', 'phase', 'next we', 'after that', 'finally', 'before we'];
const GRATITUDE = ['thanks', 'thank you', 'appreciate', 'great job', 'nice', 'perfect', 'awesome', 'lgtm', 'works'];

// Domain / topic tagging. A prompt can match multiple categories.
const CATEGORIES = {
  Frontend: ['css', 'html', 'react', 'component', 'ui', 'button', 'style', 'tailwind', 'layout', 'render', 'dom', 'jsx', 'tsx', 'frontend', 'modal', 'responsive', 'animation', 'svg'],
  Backend: ['api', 'endpoint', 'server', 'route', 'controller', 'service', 'middleware', 'request', 'response', 'backend', 'express', 'fastapi', 'handler', 'webhook'],
  Database: ['database', 'sql', 'query', 'schema', 'migration', 'table', 'postgres', 'mongo', 'redis', 'index', 'join', 'orm', 'prisma', 'column'],
  Testing: ['test', 'jest', 'spec', 'coverage', 'mock', 'assert', 'vitest', 'pytest', 'e2e', 'unit test', 'integration test'],
  Debugging: ['error', 'bug', 'crash', 'stack trace', 'undefined', 'exception', 'fails', 'failing', 'broken', 'not working', 'debug', 'traceback'],
  Refactoring: ['refactor', 'clean up', 'cleanup', 'extract', 'simplify', 'restructure', 'dedupe', 'reorganize', 'rename'],
  DevOps: ['deploy', 'docker', 'kubernetes', 'k8s', 'pipeline', 'ci/cd', 'vercel', 'aws', 'container', 'helm', 'terraform', 'nginx'],
  Git: ['git', 'commit', 'rebase', 'pull request', 'pr', 'merge conflict', 'stash', 'checkout', 'cherry-pick', 'squash'],
  Docs: ['readme', 'docstring', 'documentation', 'document the', 'comment', 'explain the', 'changelog'],
  'AI/ML': ['llm', 'embedding', 'gpt', 'claude', 'openai', 'anthropic', 'vector', 'rag', 'fine-tune', 'inference', 'prompt', 'agent'],
  Security: ['auth', 'password', 'secret', 'encrypt', 'vulnerability', 'xss', 'csrf', 'permission', 'oauth', 'jwt', 'sanitize'],
  Performance: ['performance', 'slow', 'optimize', 'latency', 'memory leak', 'bottleneck', 'speed up', 'throughput', 'lag'],
  Config: ['config', 'setup', 'install', 'dependency', 'package.json', 'npm', 'yarn', 'pnpm', 'tsconfig', 'env var', 'settings'],
};
...
const CATEGORY_RE = Object.entries(CATEGORIES).map(([cat, kws]) =>
  [cat, new RegExp('\\b(?:' + kws.map(escapeRe).join('|') + ')\\b', 'i')]);
...
export function analyze(prompts, sessionCount) {
  ...
  if (n === 0) {
    return { empty: true, message: 'No human-typed prompts found (only slash commands / tool traffic).' };
  }
  ...
  let questions = 0, polite = 0, corrections = 0, reasoning = 0, codeBlocks = 0,
      hasPath = 0, techHits = 0, longPrompts = 0, oneLiners = 0, emoji = 0,
      imperative = 0, urgency = 0, frustration = 0, autonomy = 0, planning = 0,
      gratitude = 0, multiPart = 0, hasNumbers = 0, sentencesTotal = 0, charsTotal = 0;
  ...
  const sessionTimes = {};
  let anyTs = false, minTs = Infinity, maxTs = -Infinity;
  ...
  for (const p of typed) {
    ...
    for (const [term, re] of TECH_RE) if (re.test(raw)) { techHits++; techTermCounts[term] = (techTermCounts[term] || 0) + 1; }
    if (/[\u{1F300}-\u{1FAFF}\u{2600}-\u{27BF}]/u.test(raw)) emoji++;

    const wl = w.length;
    if (wl <= 3) { lengthBuckets.terse++; oneLiners++; }
    else if (wl <= 12) lengthBuckets.short++;
    else if (wl <= 40) lengthBuckets.medium++;
    else { lengthBuckets.long++; longPrompts++; }
    ...
    if (/\n\s*\d+[.)]/.test(raw) || /\n\s*[-*]\s/.test(raw) || (t.split(/\band\b|\balso\b|;/).length - 1) >= 3) multiPart++;
  ...
  const reliability = n >= 300
    ...
    : n >= 60
      ...
      : { level: 'low', note: 'Small corpus — scores are noisy; read the prompts directly and weight lightly.' };

  const topStarters = Object.entries(firstWords).sort((a, b) => b[1] - a[1]).slice(0, 8)
    .map(([w, c]) => ({ word: w, count: c, pct: Math.round((c / n) * 100) }));

  const topTechTerms = Object.entries(techTermCounts).sort((a, b) => b[1] - a[1]).slice(0, 10)
    .map(([term, c]) => ({ term, count: c, pct: Math.round((c / n) * 100) }));

  const sessionSizes = Object.values(perSession);
  const durs = Object.values(sessionTimes).map((s) => (s.max - s.min) / 60000).filter((x) => x > 0);
  const sessionStats = {
    ...
    avgDurationMin: durs.length ? +(durs.reduce((a, b) => a + b, 0) / durs.length).toFixed(0) : null,
    longestSessionMin: durs.length ? +Math.max(...durs).toFixed(0) : null,
  };

  const dayMs = 86400000;
  const span = (anyTs && Number.isFinite(minTs))
    ? {
  ...
  const timeline = Object.entries(perDay).sort((a, b) => a[0].localeCompare(b[0]))
    .map(([date, count]) => ({ date, count }));

  return {
    empty: false,
    counts: { typedPrompts: n, slashCommands: prompts.length - n, sessions: sessionCount, totalWords },
    metrics: {
    ...
    behavior: [
      { label: 'Imperative / directive', key: 'imperative', pct: +(ratio(imperative) * 100).toFixed(1) },
    ...
    verdict: verdict(composite, depth.value, authenticity.value, independence.value, sessionStats, n),
  };
}
...
function verdict(comp, d, a, i, s, n) {
  ...
  const lead = d >= a && d >= i ? 'technical depth' : a >= i ? 'hands-on engagement' : 'balanced dialogue';
  const vol = n >= 300 ? 'a large corpus' : n >= 60 ? 'a moderate corpus' : 'a small corpus';
  return `${band} overall signal from ${vol} (${n} prompts across ${s.count} sessions). ` +
