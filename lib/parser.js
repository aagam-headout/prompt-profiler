...
import { listCodexSources, loadCodexPrompts } from './codex-parser.js';
import { listOpencodeSources, loadOpencodePrompts } from './opencode-parser.js';
...
// Unified list across tools (Claude Code projects + Cursor + Codex + OpenCode).
export function listSources() {
  ...
  let codex;
  try {
    codex = listCodexSources();
  } catch {
    codex = [];
  }
  let opencode;
  try {
    opencode = listOpencodeSources();
  } catch {
    opencode = [];
  }
...
// Dispatches Cursor/Codex/OpenCode ids to their own readers; everything else is Claude.
export function loadPrompts(projectId) {
  ...
  if (projectId && projectId.startsWith('codex::')) {
    return loadCodexPrompts(projectId);
  }
  if (projectId && projectId.startsWith('opencode::')) {
    return loadOpencodePrompts(projectId);
  }
  const dirs =
    projectId === '__all__'
