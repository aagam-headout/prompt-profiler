...
if (has('--list') || args.length === 0) {
  ...
  for (const p of sources)
    console.log(`  [${p.kind.padEnd(8)}] ${p.sessions.toString().padStart(3)} sessions  ${p.id}`);
  console.log('\nRun: node cli.js --project <id>   (or --all, or --compare a,b,c)\n');
  process.exit(0);
}

if (has('--compare')) {
  const ids = (val('--compare') || '')
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean);
  if (ids.length < 2) {
    console.error('--compare needs 2+ ids: node cli.js --compare a,b');
    process.exit(1);
  }
  const rows = ids
    .map((id) => {
...
if (!id) {
  console.error('Provide --project <id> or --all (see --list)');
  process.exit(1);
}
